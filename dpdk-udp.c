...
static void dump_ip_data(const struct rte_mbuf *mbuf, const struct ipv4_hdr *ip)
{
    ...
    switch (ip->next_proto_id) {
        ...
        case IPPROTO_UDP: {
            ...
            fprintf(stdout, "UDP[checksum 0x%x SEQ %lu op %x %s] ",
                    udp->dgram_cksum, data->seq,
                    data->op, str_op); 
        }
        break;
    }
...
static void dump_stats(struct peer *peer)
{
	printf("=== STATS for %s: %s [ sent %lu acked %lu %02.5f%%]",
	inet_ntoa(peer->addr.ip),
...
static void resolve_peer_mac(struct rte_timer *rte_tm, void *tm_arg)
{
    ...
    printf("peer %p flags 0x%lx count %d\n", peer, peer->flags, peer->resolve_count);
...
#if 0
    retval = rte_eth_dev_set_mtu(port, 9216);
    if (retval < 0) {
